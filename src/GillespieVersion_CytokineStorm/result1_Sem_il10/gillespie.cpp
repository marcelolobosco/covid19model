...
int main(){
	...
	std::array<int,n_runs> V,Ap,Apm,Thn,The,Tkn,Tke,B,Ps,Pl,Bm,IgM,IgG, //Dead
	Tnf, Il6, Il10;
	std::fill(V.begin(),V.end(),1000);
	std::fill(Ap.begin(),Ap.end(),1.0E06);
	std::fill(Apm.begin(),Apm.end(),0);
	std::fill(Thn.begin(),Thn.end(),1E06);//1.E06);
	std::fill(The.begin(),The.end(),0);
	std::fill(Tkn.begin(),Tkn.end(),5E05);//5.E05);
	std::fill(Tke.begin(),Tke.end(),0);
	std::fill(B.begin(),B.end(),1.25E05);//2.5E05);
	...
	std::fill(Tnf.begin(),Tnf.end(),0);
	std::fill(Il6.begin(),Il6.end(),0);
	std::fill(Il10.begin(),Il10.end(),0);
	...
	IgGfiles[n_runs], Tnffiles[n_runs], Il6files[n_runs], Il10files[n_runs];  //Cfiles[n_runs], Deadfiles[n_runs];
	for(int j=0; j<n_runs; j++){
		...
		iggfilename, tnffilename, il6filename, il10filename;  //cfilename, deadfilename;
		...
		tnffilename << "Tnf" << j << ".dat";
		il6filename << "Il6" << j << ".dat";
		il10filename << "Il10" << j << ".dat";
		...
		Tnffiles[j].open(tnffilename.str());
		Il6files[j].open(il6filename.str());
		Il10files[j].open(il10filename.str());
	}
	...
	for(int j=0; j<n_runs; j++){
		...
		Tnffiles[j] << Tnf[j] << endl; 
		Il6files[j] << Il6[j] << endl;
		Il10files[j] << Il10[j] << endl;
	}
	...
	for(int i=1; i<time_size; i++){
		#pragma omp parallel num_threads(4)
		{
			...
			for(int j=0; j<n_runs; j++){
				...
				&B[j], &Ps[j], &Pl[j], &Bm[j], &IgM[j], &IgG[j], &Tnf[j], &Il6[j], &Il10[j], dt);
				Vfiles[j] << V[j] << endl; 
				Apfiles[j] << Ap[j] << endl;
				Apmfiles[j] << Apm[j] << endl;
				Thnfiles[j] << Thn[j] << endl;
