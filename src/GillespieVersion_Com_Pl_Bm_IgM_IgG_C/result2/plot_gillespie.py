...
# color_index = 0
# colorVal = scalarMap.to_rgba(color_index)
# plt.figure("B cell activated")
# for k in range(0,nruns):
#     plt.plot(t, getArrayFromFile("Be"+str(k)+".dat"), label="B cell activated", color=colorVal)
#     #plt.yscale('log')
#     plt.xlabel('time (days)')
#     plt.ylabel('cells number')
#     #plt.legend()    
#     plt.title("B cell activated")
#     color_index += 20
#     colorVal = scalarMap.to_rgba(color_index)
# plt.savefig('Be.pdf', bbox_inches='tight', dpi=300)
# plt.savefig('Be.svg', format='svg', bbox_inches='tight')
...
plt.figure("IgM")
for k in range(0,nruns):
    plt.plot(t, np.log2(getArrayFromFile("IgM"+str(k)+".dat")), label="IgM", color=colorVal)    
    plt.xlabel('time (days)')
...
plt.figure("IgG")
for k in range(0,nruns):
    plt.plot(t, np.log2(getArrayFromFile("IgG"+str(k)+".dat")), label="IgG", color=colorVal)    
    plt.xlabel('time (days)')
    plt.ylabel('log2 (antibody level)')
    #plt.legend()    
    plt.title("IgG")
...
color_index = 0
colorVal = scalarMap.to_rgba(color_index)
plt.figure("Inflammatory cytokines")
for k in range(0,nruns):
    plt.plot(t, getArrayFromFile("C"+str(k)+".dat"), label="Inflammatory cytokines", color=colorVal)    
    plt.xlabel('time (days)')
    plt.ylabel('pg/ml')
    #plt.legend()    
    plt.title("Inflammatory cytokines")
    color_index += 20
    colorVal = scalarMap.to_rgba(color_index)
plt.savefig('C.pdf', bbox_inches='tight', dpi=300)
plt.savefig('C.svg', format='svg', bbox_inches='tight')