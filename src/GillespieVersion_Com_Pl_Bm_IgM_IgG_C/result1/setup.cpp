...
namespace gillespie {
    ...
    int *Ps, int *Pl, int *Bm, int *IgM, int *IgG, int *C){
		double st,x,tau;
        ...
        double dfthe = beta_Thn*(*Apm)*(*Thn)*cytokines_infl;
        double rpthe = r_the*(*Apm)*(*The)*cytokines_infl;
        double dthe = m_the*(*The);
        double hTkn = alpha_Tkn*(1)*(Tkn_max - (*Tkn));
        double aTkn = beta_Tkn*(*Apm)*(*Tkn)*cytokines_infl;
        double rpTke = r_tke*(*Apm)*(*Tke)*cytokines_infl;
        double dTke = m_tke*(*Tke);   
        double pVTke = pv_tke*(*V)*(*Tke);
        double rpB = r2_B*(*The)*(*B)*cytokines_infl;
        double hB = alpha_B*(1)*(B_max - (*B)); 
        double dfB_Ps = beta_Ps*(*Apm)*(*B)*cytokines_infl;
        double dPs = m_Ps*(*Ps);        
        ...
        + dPl + memPl + rBm + pIgG + pVIgG + dIgG + pC_Apm + pC_The + pC_Tke + dC;
    ...
    int *Ps, int *Pl, int *Bm, int *IgM, int *IgG, int *C, double time_delta){
		double time = 0.0;
		while(time < time_delta){
			time += step(V,Ap,Apm,Thn,The,Tkn,Tke,B,Ps,Pl,Bm,IgM,IgG,C);
		}
	}
