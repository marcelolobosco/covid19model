...
int main(){
	...
	std::array<int,n_runs> V,Ap,Apm,Thn,The,Tkn,Tke,B,Ps,Pl,Bm,IgM,IgG,C;	
	std::fill(V.begin(),V.end(),1000);
	std::fill(Ap.begin(),Ap.end(),1.E06);
	...
	for(int i=1; i<time_size; i++){
		#pragma omp parallel
		{
			...
			for(int j=0; j<n_runs; j++){
				...
				&B[j], &Ps[j], &Pl[j], &Bm[j], &IgM[j], &IgG[j], &C[j], dt);
				Vfiles[j] << V[j] << endl; 
				Apfiles[j] << Ap[j] << endl;
				Apmfiles[j] << Apm[j] << endl;
				Thnfiles[j] << Thn[j] << endl;
