...
if __name__ == "__main__":
    ...
    '''
    if(len(sys.argv) != 2):
        print("Usage: covid19_foward_uq.py [distributions_file]")
        sys.exit(0)
    '''   
    ...
    if (opt_storm):
        arqpar = 'execution_de_non_survivor.txt_dists.pkl'
    ...
    label_covid = ['v0', 'pi_c_apm', 'pi_c_i', 'pi_c_tke', 'delta_c', 'beta_apm',
     ...
     #--------------------Para vivos---------------------#
    dist_df = pd.read_pickle(arqpar);
    print(dist_df);
    
    dist_df =dist_df.astype(object)
    for ind in dist_df.index:
        params = dist_df['Distribution Params'][ind]
        if (dist_df['Distribution Name'][ind]=='uniform'):
            d = cp.Uniform(params[0], params[0]+params[1])
        elif(dist_df['Distribution Name'][ind]=='norm'):
            d = cp.Normal(params[0], params[1])
        elif(dist_df['Distribution Name'][ind]=='lognorm'):   
            d = cp.LogNormal(np.log(params[2]), params[0], params[1]) 
        else:
            print("Unknow distribution:"+dist_df['Distribution Name'][ind])
            sys.exit(0)
        if ind==0:
            dist = d
        else:
